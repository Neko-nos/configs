...
# Stop running this script if some error happens
set -e

# Install Homebrew
# ref: https://brew.sh/
if [[ -x "$(where brew)" ]]; then
    echo "You already installed Homebrew."
...
# Now, install the tools that can be install via brew
# define this common function to install or update a brew formula
function install_formula {
    if [[ -x "$(where ${1})" ]]; then
        echo "You already installed ${1}."
        printf "Update ${1}? [y/N]: "
        if read -q; then
            echo; brew upgrade ${1}
        fi
    # some formulae fail in the above case (command not found etc)
    # brew info command takes more time than where command, so we use it only after the above case fail

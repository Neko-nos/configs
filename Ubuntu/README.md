...
- Enable password-less `sudo` execution\
  [How to use sudo without entering a password - Qiita](https://qiita.com/RyodoTanaka/items/e9b15d579d17651650b7)

- Run `sudo apt update && sudo apt upgrade -y`

- Change the position of the Dock and other appearance elements for a Mac-like layout\
  [Initial settings for Ubuntu (MacOs-tyle) - Qiita](https://qiita.com/momokura/items/33cd6ee525553fc91473)

  - You can also use `dconf-editor` (or `gsettings`, but if you're going to use a GUI anyway, scripting becomes a hassle) to reposition applications\
...
- Install a text editor (like VSCode) to prepare for editing configuration files
  - For VSCode, run `uname -m` to check your architecture (x86_64 etc) and download the correct `.deb` file\
    [VSCode on Linux - Official Site](https://code.visualstudio.com/docs/setup/linux)\
...
- Enhance clipboard functionality (equivalent to Clipy on Mac or Clibor on Windows)
  - Use the following GNOME extension:\
    [Clipboard Indicator GitHub](https://github.com/Tudmotu/gnome-shell-extension-clipboard-indicator)\
...
The goal is to make key mappings feel like MacOS and let's use `xkb`, `input-remapper`, and `AutoKey` to achieve this.\
...
[xkb key remapping in Ubuntu - Honmushi blog](https://honmushi.com/2019/01/18/ubuntu-xkb/)\
`xmodmap` used to be mainstream, but now somehow the settings for `xmodmap` are frequently overwritten on some background process, so it's better to use `xkb` instead.\
...
The middle part of the numeric key represents the key code. You can refer to the values in `xmodmap.json`\
...
[AutoKey Official Docs](https://autokey.github.io/index.html)\
...
- Remap `Henkan/Muhenkan` keys to IME activation/deactivation for a Mac-like experience\
...
- Emulate MacOS-style shortcuts using `Ctrl` and `Command`-like behavior
  ...
  - After that, change the `Super + *` shortcuts in Ubuntu settings to require Shift.
    - If you remap `Eisu toggle` to `Super L`, use `gnome-tweaks` to assign `Super_R` to the Activities Overview\
      [Want to disable the 'Activities' screen opening on single press of Super key in Ubuntu - Neo's World](https://neos21.net/blog/2020/03/23-03.html)\
...
- Referenced the following article for shortcuts:
  - Could use `<ctrl>` + key, but want to use outside the terminal too, so if possible directly with a different key (like `Home`), use that\

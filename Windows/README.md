...
Windows TerminalからWSL内のCodex CLIを使う場合、Shift+Enterが通常のEnterと区別されずに渡ることがある。\
Windows Terminalの`settings.json`には[`Windows/Terminal/settings_snippet.jsonc`](./Terminal/settings_snippet.jsonc)の`actions`を追加し、Shift+EnterでCtrl+Jを送るようにしている。\
